...
const LANG_STRINGS = {
    en: {
        ...
        'kanban.manageProjects': 'Projects',
        'kanban.noProjectsYet': 'No projects yet - use the + button to create one.',
        'kanban.renameProject': 'Rename',
        'kanban.deleteProject': 'Delete',
        'kanban.deleteProjectConfirm': 'Delete the project "{name}"? Items inside it will stay, just untagged from any project.',
        'notes.imageMenuCopy': 'Copy',
        'notes.imageMenuCut': 'Cut',
        'notes.imageMenuWhatsApp': 'Send to WhatsApp',
        'notes.imageMenuMoveUp': 'Move up',
        'notes.imageMenuMoveDown': 'Move down',
        'notes.imageMenuDelete': 'Delete',
        'kanban.done': 'Done',
        ...
        'notes.downloadShort': 'Download all notes',
        'notes.downloadThisNote': 'Download this note',
    ...
    es: {
        ...
        'kanban.manageProjects': 'Proyectos',
        'kanban.noProjectsYet': 'Aún no hay proyectos - usa el botón + para crear uno.',
        'kanban.renameProject': 'Renombrar',
        'kanban.deleteProject': 'Eliminar',
        'kanban.deleteProjectConfirm': '¿Eliminar el proyecto "{name}"? Las tarjetas que tenía se quedan, solo quedan sin proyecto.',
        'notes.imageMenuCopy': 'Copiar',
        'notes.imageMenuCut': 'Cortar',
        'notes.imageMenuWhatsApp': 'Enviar a WhatsApp',
        'notes.imageMenuMoveUp': 'Mover arriba',
        'notes.imageMenuMoveDown': 'Mover abajo',
        'notes.imageMenuDelete': 'Eliminar',
        'kanban.done': 'Hecho',
        ...
        'notes.downloadShort': 'Descargar todas las notas',
        'notes.downloadThisNote': 'Descargar esta nota',
        ...
        'header.privacyPolicy': 'Política de privacidad',
...
// One floating save/undo/redo group everywhere - including Notes now,
// which used to have its own separate, always-visible undo/redo pair
// in its own toolbar instead. This button now routes to whichever
// undo/redo actually applies: the Notes-specific stack (notesUndoStack/
// notesRedoStack) whenever a note is open, or the calendar/backlog
// stack (undoStack/redoStack, same as desktop Ctrl+Z) otherwise - see
// isNotesUndoContextActive()/handleGlobalUndoClick()/
// handleGlobalRedoClick() below. Called after every snapshot push/undo/
// redo (either kind), and once on load/tab-switch from
// setMobileSection() so it's correct as soon as the page opens.
function isNotesUndoContextActive() {
    return !!currentNoteId;
}
function handleGlobalUndoClick() {
    if (isNotesUndoContextActive()) undoNoteAction();
    else undoLastAction();
}
function handleGlobalRedoClick() {
    if (isNotesUndoContextActive()) redoNoteAction();
    else redoLastAction();
}
function updateGlobalUndoBtn() {
    const wrap = document.getElementById('mobile-global-undo');
    if (!wrap) return;
    // On phone, a note's own toolbar floats at the same bottom corner as
    // this group - raised out of the way there so the two don't overlap
    // (see .raised-for-notes). Harmless on desktop, where that toolbar
    // isn't fixed-positioned in the first place.
    wrap.classList.toggle('raised-for-notes', currentMobileSection === 'notes');
    const undoBtn = document.getElementById('mobile-global-undo-btn');
    const redoBtn = document.getElementById('mobile-global-redo-btn');
    if (isNotesUndoContextActive()) {
...
function renderVisualMatrix() {
    ...
    // Mirrors the corner "Time" header on the right edge too, matching
    // the extra time column appended after the day columns below - same
    // reasoning as the Today strip (see renderTodayStrip): once this
    // grid is scrolled sideways or the screen is narrow, the hour
    // labels on the left scroll out of view first, so having them on
    // the right too means you're never without them.
    ...
    // Right-edge mirror of the left time column (see the header mirror above).
    const timeColRight = document.createElement('div');
    timeColRight.className = 'matrix-time-col';
    for (let h = 0; h < 24; h++) {
        const slot = document.createElement('div');
        slot.className = 'time-slot-label';
        slot.innerText = `${String(h).padStart(2, '0')}:00`;
    ...
    matrixGrid.appendChild(timeColRight);
}
...
document.addEventListener('keydown', (e) => {
    ...
    if (!isTyping && ctrlOrCmd && e.key.toLowerCase() === 'z') {
        if (e.shiftKey) {
            handleGlobalRedoClick();
        } else {
            handleGlobalUndoClick();
        }
        e.preventDefault();
    }
...
function saveBoardProjects() {
    ...
    queueCloudSync(); // a project created with no items in it yet had nothing else to trigger a sync, so it could vanish on the next cloud pull from another device
}

function renderBoardProjectManagerLabel() {
    const label = document.getElementById('board-project-manager-label');
    if (label) label.textContent = activeBoardProject || t('kanban.allProjects');
}
...
// --- BOARD PROJECT MANAGER (see, select, rename, delete) ---
// Everything project-related lives in this one dropdown now: the
// trigger button shows whichever project is active (or "All"), the
// panel opens to an "add a new project" field plus the full list -
// tap a name to switch to it and close, ✎ to rename inline, 🗑 to
// delete. Same interaction pattern as the Categories manager.
function toggleBoardProjectManagerDropdown() {
    const panel = document.getElementById('board-project-manager-panel');
    const trigger = document.getElementById('board-project-manager-trigger');
    if (!panel) return;
    const opening = !panel.classList.contains('open');
    panel.classList.toggle('open', opening);
    if (trigger) trigger.classList.toggle('open', opening);
    if (opening) renderBoardProjectManagerDropdown();
}
document.addEventListener('click', (e) => {
    const manager = document.getElementById('board-project-manager');
    const panel = document.getElementById('board-project-manager-panel');
    if (!manager || !panel || !panel.classList.contains('open')) return;
    if (!manager.contains(e.target)) {
        panel.classList.remove('open');
        document.getElementById('board-project-manager-trigger')?.classList.remove('open');
    }
});

function renderBoardProjectManagerDropdown() {
    const list = document.getElementById('board-project-manager-list');
    if (!list) return;
    list.innerHTML = '';

    const allRow = document.createElement('div');
    allRow.className = 'category-manager-row';
    allRow.innerHTML = `<span class="category-manager-name" style="cursor:pointer;">${activeBoardProject === '' ? '✓ ' : ''}${t('kanban.allProjects')}</span>`;
    allRow.querySelector('.category-manager-name').addEventListener('click', () => {
        setActiveBoardProject('');
        toggleBoardProjectManagerDropdown();
    });
    ...
    if (boardProjects.length === 0) {
        ...
        empty.textContent = t('kanban.noProjectsYet');
        list.appendChild(empty);
        return;
    }

    boardProjects.forEach(proj => {
        const count = backlogItems.filter(i => (i.project || '') === proj).length;
        const isActive = activeBoardProject === proj;
        const row = document.createElement('div');
        row.className = 'category-manager-row';
        row.innerHTML = `
            <span class="category-manager-name" title="${escapeHtml(proj)}">${isActive ? '✓ ' : ''}${escapeHtml(proj)} <span style="opacity:0.6; font-weight:400;">(${count})</span></span>
            <button type="button" class="category-manager-del-btn" title="${t('kanban.renameProject')}">✎</button>
            <button type="button" class="category-manager-del-btn" title="${t('kanban.deleteProject')}">🗑</button>
        `;
        const [renameBtn, delBtn] = row.querySelectorAll('.category-manager-del-btn');
        row.querySelector('.category-manager-name').addEventListener('click', () => {
            setActiveBoardProject(proj);
            toggleBoardProjectManagerDropdown();
        });
        renameBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            startRenameBoardProjectInline(proj, row);
        });
        delBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            deleteBoardProject(proj);
        });
        list.appendChild(row);
    });
}

function startRenameBoardProjectInline(name, rowEl) {
    const nameEl = rowEl.querySelector('.category-manager-name');
    if (!nameEl) return;
    const input = document.createElement('input');
    input.type = 'text';
    ...
    const commit = () => {
        const newName = input.value.trim();
        if (newName && newName !== name && !boardProjects.includes(newName)) {
            renameBoardProjectEverywhere(name, newName);
        }
        renderBoardProjectManagerDropdown();
    };
    input.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') commit();
        if (e.key === 'Escape') renderBoardProjectManagerDropdown();
    });
    input.addEventListener('blur', commit);
    nameEl.replaceWith(input);
    input.focus();
...
// Renames a project everywhere it's referenced: the master list, the
// active filter (if this was the one selected), and every backlog/board
// item tagged with the old name - otherwise those items would silently
// fall back to "No project" the moment the name changed.
function renameBoardProjectEverywhere(oldName, newName) {
    boardProjects = boardProjects.map(p => p === oldName ? newName : p);
    backlogItems.forEach(item => { if (item.project === oldName) item.project = newName; });
    if (activeBoardProject === oldName) {
        activeBoardProject = newName;
        localStorage.setItem('activeBoardProject', newName);
    }
    saveBoardProjects();
    localStorage.setItem('backlogItems', JSON.stringify(backlogItems));
    queueCloudSync();
...
async function deleteBoardProject(name) {
    const ok = await showConfirmDialog(t('kanban.deleteProjectConfirm', { name }));
    if (!ok) return;
    boardProjects = boardProjects.filter(p => p !== name);
    backlogItems.forEach(item => { if (item.project === name) item.project = null; });
    if (activeBoardProject === name) {
        activeBoardProject = '';
        localStorage.setItem('activeBoardProject', '');
    }
    saveBoardProjects();
    localStorage.setItem('backlogItems', JSON.stringify(backlogItems));
    queueCloudSync();
...
function renderBoardSection() {
    renderBoardProjectManagerLabel();
    if (document.getElementById('board-project-manager-panel')?.classList.contains('open')) {
        renderBoardProjectManagerDropdown();
    }
...
// Kept as a thin alias: notes.js call sites (pushNotesUndoSnapshot,
// applyUndoRedoResult) call this by name after every notes undo-related
// change. The dedicated #notes-undo-btn/#notes-redo-btn pair that used
// to live in the notes toolbar is gone now - the floating global group
// (#mobile-global-undo) is the one and only undo/redo control
// everywhere, including Notes - so this just refreshes that instead.
...
function showMobileNotesList() {
    flushNoteSave(); // don't lose a freshly-typed title/notebook name or in-progress edit when backing out on phone
    stopNoteRecordingIfActive();
    setMobileNotesView('list');
}
...
// The actual cause of recordings quietly capping out after ~12-17
// seconds: most phones auto-lock the screen after a short idle timeout,
// and holding the phone up to talk doesn't count as "activity" that
// resets it - so on many devices the screen (and the tab with it) goes
// hidden mid-recording. When that happens, the visibilitychange handler
// below used to just kill the mic stream to save battery, silently
// leaving the recording "running" with no audio coming in - so the
// clip you got back was truncated to whatever was captured before the
// screen turned off, with no indication why. Holding a Screen Wake
...
async function acquireNoteRecordingWakeLock() {
    ...
    try {
        noteRecordingWakeLock = await navigator.wakeLock.request('screen');
        // Some browsers release the lock on their own (e.g. the OS pulled
        // it back for power reasons) without the page ever going hidden -
        // reflect that so a stale reference doesn't linger.
        noteRecordingWakeLock.addEventListener('release', () => {
            noteRecordingWakeLock = null;
        });
    } catch (err) {
        // Can fail for reasons outside our control (battery saver,
        // permissions policy) - recording continues regardless, this is
        // just the "keep the screen on" nicety on top of it.
        noteRecordingWakeLock = null;
    }
}
function releaseNoteRecordingWakeLock() {
    if (noteRecordingWakeLock) {
        try { noteRecordingWakeLock.release(); } catch (e) {}
        noteRecordingWakeLock = null;
    }
}
...
// Inserts a full-width divider line - the toolbar button version of
// typing a row of dashes to separate ideas, which is common enough to
// deserve one click. This actually splits the focused text block AT
// THE CURSOR when there's a live text selection to split (see
// splitFocusedTextBlockForInsert below) - previously it only ever
// landed after the WHOLE focused block, which for a single running
// text block (the common case: one note, several paragraphs, no
// separate blocks yet) looked exactly like "it always goes to the
// bottom" the moment the cursor wasn't already at the very end.
function insertDividerBlock() {
    ...
    const { insertAt, focusBlockId: splitFocusId } = splitFocusedTextBlockForInsert(note);

    note.blocks.splice(insertAt, 0, dividerBlock);

    let focusBlockId = splitFocusId;
    if (!focusBlockId) {
        // Cursor was at the very end (or nothing was focused) - same as
        // before, drop a fresh empty block right after to keep typing in.
        const newTextBlock = { id: genBlockId(), type: 'text', content: '' };
        note.blocks.splice(insertAt + 1, 0, newTextBlock);
        focusBlockId = newTextBlock.id;
    }
    lastFocusedBlockId = focusBlockId;

    renderNoteBlocks(note);
    scheduleNoteSave(note);
    renderNotesList();

    const newTa = document.querySelector(`textarea[data-block-id="${focusBlockId}"]`);
    if (newTa) {
        newTa.focus();
        if (splitFocusId) newTa.setSelectionRange(0, 0); // land at the start of the text that used to continue right here
    }
}

// Shared by anything inserted via the toolbar (divider today; photos/
// voice notes could use this too) that wants to land exactly at the
// cursor instead of always after the whole focused block. When the
// focused block is a text block AND its textarea actually has focus
// with a live caret position that's strictly inside its content, the
// block is split into two right there - the "before" half keeps the
// original block, the "after" half becomes a new block, and whatever
// gets inserted goes between them. Otherwise (cursor at the very
// start/end, a non-text block focused, or nothing focused) this just
// returns the plain "insert after the focused block" position, exactly
// like before.
function splitFocusedTextBlockForInsert(note) {
    ensureNoteBlocks(note);
    if (!lastFocusedBlockId) return { insertAt: note.blocks.length, focusBlockId: null };

    const idx = note.blocks.findIndex(b => b.id === lastFocusedBlockId);
    if (idx === -1) return { insertAt: note.blocks.length, focusBlockId: null };

    const block = note.blocks[idx];
    if (block.type !== 'text') return { insertAt: idx + 1, focusBlockId: null };

    const ta = document.querySelector(`textarea[data-block-id="${block.id}"]`);
    if (!ta || document.activeElement !== ta || typeof ta.selectionStart !== 'number') {
        return { insertAt: idx + 1, focusBlockId: null };
    }

    const pos = ta.selectionStart;
    const content = block.content || '';
    if (pos <= 0 || pos >= content.length) {
        // Cursor already at the very start or end - nothing meaningful to
        // split, insert before/after this block same as always.
        return { insertAt: pos <= 0 ? idx : idx + 1, focusBlockId: null };
    }

    const before = content.slice(0, pos);
    const after = content.slice(pos);
    block.content = before;
    const afterBlock = { id: genBlockId(), type: 'text', content: after };
    note.blocks.splice(idx + 1, 0, afterBlock);
    return { insertAt: idx + 1, focusBlockId: afterBlock.id };
}
...
function buildImageBlockEl(block) {
    ...
    //
    // The ⋮ menu in the corner is the mobile-reachable version of all of
    // that - Ctrl+C and Delete-key don't exist on a phone, so without it
    // there was no touch-friendly way to delete, copy, or reorder a
    // photo at all.
    wrap.innerHTML = `
        ...
        <div class="note-image-block-menu-wrap">
            <button type="button" class="note-image-menu-btn" title="Photo options">⋮</button>
            <div class="note-image-menu-panel">
                <button type="button" class="note-image-menu-item" data-action="copy">📋 ${t('notes.imageMenuCopy')}</button>
                <button type="button" class="note-image-menu-item" data-action="cut">✂️ ${t('notes.imageMenuCut')}</button>
                <button type="button" class="note-image-menu-item" data-action="whatsapp">💬 ${t('notes.imageMenuWhatsApp')}</button>
                <button type="button" class="note-image-menu-item" data-action="up">↑ ${t('notes.imageMenuMoveUp')}</button>
                <button type="button" class="note-image-menu-item" data-action="down">↓ ${t('notes.imageMenuMoveDown')}</button>
                <div class="note-image-menu-divider"></div>
                <button type="button" class="note-image-menu-item note-image-menu-danger" data-action="delete">🗑️ ${t('notes.imageMenuDelete')}</button>
            </div>
        </div>
    `;
    ...
    const menuWrap = wrap.querySelector('.note-image-block-menu-wrap');
    const menuBtn = wrap.querySelector('.note-image-menu-btn');
    menuBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        const opening = !menuWrap.classList.contains('open');
        closeAllImageBlockMenus();
        menuWrap.classList.toggle('open', opening);
    });
    wrap.querySelectorAll('.note-image-menu-item').forEach(item => {
        item.addEventListener('click', (e) => {
            e.stopPropagation();
            menuWrap.classList.remove('open');
            handleImageBlockMenuAction(item.dataset.action, block);
        });
    });
...
// Only one photo's menu open at a time, and closes on any click
// elsewhere - same convention as every other dropdown in the app
// (category manager, board project manager, notebook filter).
function closeAllImageBlockMenus() {
    document.querySelectorAll('.note-image-block-menu-wrap.open').forEach(el => el.classList.remove('open'));
}
document.addEventListener('click', (e) => {
    if (!e.target.closest('.note-image-block-menu-wrap')) closeAllImageBlockMenus();
});

function handleImageBlockMenuAction(action, block) {
    if (action === 'copy') copyImageBlockToClipboard(block);
    else if (action === 'cut') cutImageBlock(block);
    else if (action === 'whatsapp') shareImageBlockToWhatsApp(block);
    else if (action === 'up') moveBlock(block.id, -1);
    else if (action === 'down') moveBlock(block.id, 1);
    else if (action === 'delete') deleteNoteBlock(block.id);
}

// Copies the photo to the clipboard, then removes it - the standard
// "cut" contract (unlike Delete, this doesn't ask for confirmation
// first: choosing Cut from the menu is already the deliberate action,
// the same way Ctrl+X never asks "are you sure?" in any other editor).
// Still goes through Trash rather than vanishing outright, so it's
// recoverable there for 30 days like any other deleted block.
async function cutImageBlock(block) {
    await copyImageBlockToClipboard(block);
    const note = notesData.find(n => n.id === currentNoteId);
    if (!note) return;
    const stillThere = note.blocks.find(b => b.id === block.id);
    if (!stillThere) return;
    pushNotesUndoSnapshot();
    pushToTrash({ type: 'block', data: JSON.parse(JSON.stringify(stillThere)), noteId: note.id, noteTitle: note.title || 'Untitled note' });
    note.blocks = note.blocks.filter(b => b.id !== block.id);
    if (note.blocks.length === 0) note.blocks.push({ id: genBlockId(), type: 'text', content: '' });
    renderNoteBlocks(note);
    scheduleNoteSave(note);
    renderNotesList();
}

// Hands the actual photo bytes to the OS share sheet so WhatsApp (or
// whatever the person picks) receives the real image, not just a text
// link - a wa.me link can't carry image data, so that's not an option
// here the way it is for sharing a note's plain text.
async function shareImageBlockToWhatsApp(block) {
    try {
    ...
    // No Web Share support for files on this browser (or it failed) -
    // opening the photo directly is the most reliable fallback, since it
    // can be saved or shared manually from there.
    window.open(block.imageData, '_blank');
    setVoiceStatus("This browser can't attach photos directly - opened it in a new tab so you can save and share it from there.");
    setTimeout(() => setVoiceStatus(''), 4000);
}
...
function selectNote(id) {
    ...
    updateGlobalUndoBtn(); // now that currentNoteId changed, the floating undo/redo should reflect THIS note's history, not whatever was active before
...
function flushNoteSave() {
    // A brand-new notebook name typed in the inline field only used to
    // get registered on blur/Enter (commitNoteNotebook via onchange) -
    // fine on desktop, but several mobile navigation paths (like the
    // "←" back button) swap views without ever firing a real blur, so
    // the name stuck to that one note but never made it into the master
    // notebooks list. flushNoteSave runs on every one of those "leaving
    // the note" paths already, so committing it here catches all of
    // them.
    commitNoteNotebook();
...
function commitNoteNotebook() {
    if (!currentNoteId) return;
    const input = document.getElementById('note-category-input');
    const notebookValue = input ? input.value.trim() : '';
...
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
        ...
        emergencySaveOnExit();
        if (noteRecordingIsActive) {
            // The wake lock above should prevent this from firing off a
            // plain screen timeout, but a manual power-button lock or an
            // actual app switch still can. Rather than silently killing
            // the mic and leaving the recording truncated with no
            // explanation (the old behavior), properly stop and save
            // whatever was captured so nothing is lost outright.
            stopNoteRecordingIfActive();
            setVoiceStatus("Recording saved - the screen turned off or you left the app.");
            setTimeout(() => setVoiceStatus(''), 4000);
        } else {
